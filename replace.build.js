...
var isArcs = app.toUpperCase() == "ARCS" || app.toUpperCase() == "AZURE" || app.toUpperCase() == "QMH" || app.toUpperCase() === "KONE";
...
async function main() {
  try {
    ...
    try {
      ...
      //   });
      // }else {
        fs.copyFile(`src/assets/config/config_${app.toLowerCase()}.json`, 'src/assets/config/config.json', async (err) => {
          if (err) {
            console.log('Warning : Config File Not Replaced');
            console.log(err)
            throw err;
