...
import { JMap, MapJData } from 'src/app/services/data.models';
...
@ViewChild('buttonGroup') buttonGroupRef : ButtonGroup
  ...
  async delete(){
    ...
    let resp = await this.dataSrv.deleteRecords( this.selectedTab == 'map' ? 'api/map/v1' : 'api/map/plan/v1',   this.data.filter(r => r['select'] == true))
    if (resp == true) {
      this.loadData()
    }
  }
