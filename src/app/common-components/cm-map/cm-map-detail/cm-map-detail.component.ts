...
export class CmMapDetailComponent implements OnInit {
  ...
  ngOnDestroy(){
    this.onDestroy.next()
    this.changeBackModeToNavigation()
    this.dataSrv.unsubscribeSignalRs(this.signalRSubscribedTopics)
  ...
  async listenScanMap_SA(){
    ...
    let robot : Robot
    this.signalRSubscribedTopics = ['occupancyGridMap' , 'pose' , 'state']
    await this.dataSrv.subscribeSignalRs(this.signalRSubscribedTopics)
   
    this.dataSrv.signalRSubj.occupancyGridMap.pipe(skip(1) ,filter(o=>o?.['data'], debounceTime(1000)), takeUntil(this.onDestroy)).subscribe(async(o)=>{
