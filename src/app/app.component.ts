...
export class AppComponent implements OnInit, OnDestroy {
  ...
  constructor(
    @Inject("SSR_APP_SETTINGS") ssrAppSettings,
  ...
  ngOnInit() {
    ...
    this.router.events.subscribe((route: NavigationStart) => {
      ...
      this.uiSrv.arcsTabletMode =
        this.currentRoute.toLowerCase() == "/waypoint" ? "WAYPOINT" : null;
    });
  ...
  private getRobotTypePaths() {
    ...
    environment.routes
      .filter((r) =>
        this.util.config.ROBOT_TYPE?.map(
          (t) => "/" + t.toLowerCase()
        )?.includes(r.path)
      )
      .forEach((r) => {
        ret.push({
          text: this.uiSrv.translate(r.text),
          icon: r.icon,
          path: r.path,
          selected: this.isDrawerItemSelected(r.path),
        });
      });
    return ret;
  }
  ...
  public drawerItems(robotTyes: string[] | null = null) {
    ...
    if (environment.app.toUpperCase() == "STANDALONE") {
      return [
        {
          text: this.uiSrv.translate("Dashboard"),
          icon: "mdi mdi-collage",
          path: "/dashboard",
          selected: selected("/dashboard"),
        },
        {
          text: this.uiSrv.translate("Task"),
          icon: "mdi mdi-clipboard",
          path: "/task",
          selected: selected("/task"),
        },
        {
          text: this.uiSrv.translate("Map"),
          icon: "mdi mdi-map",
          path: "/map",
          selected: selected("/map"),
        },
        {
          text: this.uiSrv.translate("Control"),
          icon: "mdi mdi-cogs",
          path: "/control",
        ...
        (itm) =>
          this.authSrv.hasAccessToPath(itm.path.replace("/", "")) &&
          (itm.path != "/dashboard" || this.uiSrv.withDashboard)
      );
      ...
      return [
        ...
        .concat(
          this.getRobotTypePaths().filter((p) =>
            Object.keys(this.uiSrv.robotTypeIconMap)
              .map((t) => t.toLowerCase())
              .includes(p.path.replace("/", ""))
          )
        )
        .concat(
          [
            {
              text: this.uiSrv.translate("Setup"),
              icon: "mdi mdi-cogs",
              path: "/setup",
  ...
  public onSelect(ev: DrawerSelectEvent): void {
    this.router.navigate(
      ...
      this.routeSrv.queryParams?.value?.selectedTab
        ? {
            queryParams: {
              selectedTab: this.routeSrv.queryParams?.value?.selectedTab,
            },
          }
        : undefined
    );
